	...
	this.draw = function(){
 	...
 	this.collide = function(p, q) {
		if (p + 33 > this.a && p + 33 < this.a + 7.5 && q > this.b - 8.1 && q < this.b + 20) {
			score += 2; // Increase score by 2 when the balloon is hit
			nArrows--;
			balloonPop.play();
			this.a = Math.random() * 150 + 110;
			this.b = 150;
...
function Velocity() {
    if (score < 3) {
        ...
        return (Math.random() * 0.5 + 1.2); // Adjust velocity to a moderate value
    }
    return (2.1); // Default velocity
}
...
function animate() {
    ...
    for (var i = 0; i < balloonArray.length; i++) {
        ...
        if (balloonArray[i].b < 0 && !balloonArray[i].popped) {
            score -= 1; // Subtract 1 point when a balloon is missed
            balloonArray[i].popped = true; // Mark the balloon as missed
        }
